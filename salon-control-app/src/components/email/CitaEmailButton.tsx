// src/components/CitaEmailButton.tsx - Botón de email para integrar en las citas
...
export default function CitaEmailButton({ 
  ...
  const { sendReminder, getStatus } = useEmailService();
  ...
  const isEmailConfigured = getStatus().isConfigured;
...
// Hook para usar en componentes de citas
export const useCitaEmail = () => {
  const { sendReminder, getStatus } = useEmailService();

  const sendQuickReminder = async (
    cita: any,
    cliente: any,
    servicio: any,
    type: 'confirmacion' | 'recordatorio' | 'cambio' = 'recordatorio'
  ) => {
    if (!cliente.email) {
      return { success: false, error: 'Cliente no tiene email registrado' };
    }

    if (!getStatus().isConfigured) {
  ...
  return {
    sendQuickReminder,
    isConfigured: getStatus().isConfigured
  };
};